    ...
    class Program
    {
        ...
        public static async Task CallAllPlayBeforeAppointment(string BaseUrl)
        {
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage clientResponse = await client.GetAsync("api/Play/GetAllPitchBookingPrior48hrsToStartDate");
        ...
        {
            var clientResponse = await client.GetAsync($"api/Play/PaymentAuthProcess/{bookingId}");
            if (clientResponse.IsSuccessStatusCode)
            {
                var response = await clientResponse.Content.ReadAsStringAsync();
