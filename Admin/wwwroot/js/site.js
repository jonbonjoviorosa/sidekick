...
function validateFacilityImages(inputId) {
    var input = document.getElementById(inputId);
    var files = input.files;
    ...
    if (ext.split('/')[0] === 'image') {
        ...
        if (inputId.includes("Banner") || inputId.includes("Icon") || inputId.includes("ImageUrl") || inputId.includes("ProfileImage")) {
            if (inputId.includes("Icon")) {
                uploadURL = BASEPATH + "/Attachment/" + 'UploadIconImage';
            }
            else {
            ...
            reader.onload = function (frEvent) {
                ...
                image.onload = function () {
                    ...
                    uploadFacilityImages(inputId, uploadURL);
...
function uploadFacilityImages(inputId, uploadURL) {
    var input = document.getElementById(inputId);
    var files = input.files;
    var formData = new FormData();
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                //for cleanup
                else if (inputId.includes("Icon")) {
                    if (inputId.includes('EditSpecialtyIconImage')) {
                        $('#edited-specialty-icon').val(data.publicUrl);
                        $("#editSpecialtyIconImage").attr("src", data.publicUrl);
                        $("#editSpecialtyIconImage").show();
                    }
                    else if (inputId.includes('EditSportIconImage')) {
                        $('#edited-sport-icon').val(data.publicUrl);
                        $("#imgPrevEditSportIconImage").attr("src", data.publicUrl);
                        $("#imgPrevEditSportIconImage").show();
                    }
                    else if (inputId.includes('UploadGymIconImage')) {
                        $('#gym-icon').val(data.publicUrl);
                    }
                    else if (inputId.includes("EditGymIconImage")) {
                        $('#edited-gym-icon').val(data.publicUrl);
                        $("#imgPrevEditGymIconImage").attr("src", data.publicUrl);
                        $("#imgPrevEditGymIconImage").show();
                    }
                    else if (inputId.includes('UploadIconImage')) {
                        $('#specialty-icon').val(data.publicUrl);
                    }
                    else {
                        $('#Icon').val(data.publicUrl);
                    }                      
                }
                else {
                    $("#ImageUrl").val(data.publicUrl);
